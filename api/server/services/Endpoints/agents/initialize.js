...
/**
 * Creates a permission checker with custom RBAC logic for department-based access.
 * This wraps the standard checkPermission to add department/category matching.
 * @param {Express.Request} req - The request object
 * @param {Function} originalCheckPermission - The original checkPermission function
 * @returns {Function} A wrapped permission checker with RBAC logic
 */
function createRbacPermissionChecker(req, originalCheckPermission) {
  return async function checkPermissionWithRbac({
    userId,
    role,
    resourceType,
    resourceId,
    ...
    let hasAccess = await originalCheckPermission({
      userId,
      role,
      resourceType,
      resourceId,
    ...
    // If standard ACL fails and user is not ADMIN, check department-based access
    if (!hasAccess && role !== 'ADMIN') {
      ...
      // Fetch the agent to check its category
      const agent = await db.getAgent({ id: resourceId });
      if (agent) {
...
const initializeClient = async ({ req, res, signal, endpointOption }) => {
  ...
  // Create RBAC-enhanced permission checker for discoverConnectedAgents
  const checkPermissionWithRbac = createRbacPermissionChecker(req, checkPermission);
  ...
  const {
    ...
    },
    {
      ...
      checkPermission: checkPermissionWithRbac, // Use RBAC-enhanced checker
      logViolation,
      db: {
        getFiles: db.getFiles,
        getUserKey: db.getUserKey,
